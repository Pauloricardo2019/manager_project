...
type CreateFn func(user *model.User) error
...
type GetUsersByCompanyIDFn func(companyID uint64) ([]model.User, error)

func GetUsersByCompanyID(companyID uint64) ([]model.User, error) {
	db, err := sql.GetGormDB()
	if err != nil {
		return nil, err
	}
